...
| 插件工具桥：她能不能调**别的插件**（按分级、按通道） | ✅ 落地（§4.0ag）：四道闸 + 拖拽分档界面；取数改走宿主 HTTP（SDK 的 `ctx.query_plugins` 在本机根本不工作）。**待现场复测**：这台机器只注册了 qq_auto_reply 一个插件 |
...
真机（14:36:05）：`插件工具桥候选: 0 个可用；宿主目录 1 项，状态分布 {'running': 1}`
—— 取数通了、状态读到了、QQ 家族被排掉了。**这台机器上宿主只注册了一个插件
（就是 qq_auto_reply 自己）**，所以"真的挂上别的插件的工具"这一步还没有现场样本：
需要在宿主的插件管理里再启用一个非 QQ 插件（点歌/搜索/米家等），然后她那一轮的
system 段里才会出现 `plugin_<id>` 工具。
