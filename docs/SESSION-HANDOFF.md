...
**最新：806 passed**（见 §4.0p / §4.0q / §4.0r / §4.0s / §4.0t / §4.0u / §4.0v / §4.0w）。
...
| 引用链里的图（`_build_message_chain` 的 image 分支） | ✅ 助手已通（实测 1.0–2.5s/8s 预算）+ 补 9 条看门狗 + 补留痕；**无现场证据**（见 §4.0w） |
...
**未验证（说清楚）**：**没有真实"引用了带图消息"的现场证据** —— 要群里真有人引用一张图
才能看到。确认方式就是 UI 运行时日志里那条 `[VLM] 引用图描述: …`。
