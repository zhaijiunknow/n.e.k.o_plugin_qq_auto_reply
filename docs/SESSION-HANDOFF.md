...
| 「结果出来我告诉你」是句**空话**（异步 entry 的结果没人回来喂） | ✅ **真机已通**（§4.0ah）：后台轮询 + 主动回投，16:45/16:48 两次真机把她该说的话发进了 QQ。未真机覆盖：登记那一段（要 LLM 真去调一次）、"完成"分支（这台机器上没有能跑完的异步任务） |
...
**真机没覆盖到的两段**（要说清楚）：
* **登记那一段**：真机上要触发它得让 LLM 真去调一次异步 entry（这条路要么等使用者在 QQ 上
  说一句，要么等哪次真调用）。兜底证据是：handler 集成测试 + 17/17 变异。
* **"完成"那条分支**：这台机器上**没有能跑完的异步任务**（唯一的异步生产者
  `analyze_text` 的模型端点调不通，`music_pusher` 那个 `task_id` 是排程任务不是作业），
  所以 done 分支只有单测与变异证据，没有真机日志。
