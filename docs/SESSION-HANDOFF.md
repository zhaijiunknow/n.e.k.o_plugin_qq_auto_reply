...
### 4.0p status.html 的刷新够不着（使用者："很反人类"）

**问题**：`status.html` 有 **6 张卡片**（连接方式 / 开机自启 / 一键部署 / 登录二维码 /
扫码绑定 / 信任名单），而两个刷新按钮只在顶部 `<header>` 里 —— 滑到底想刷新得先一路
滚回顶。

**修法**：加一个 `position:fixed` 的浮动刷新按钮，滚过顶部那排按钮（`scrollY > 140`）
之后才出现，免得同屏两个"刷新"。样式沿用本体浮动控件的语言（半透明白 + 分层投影）。
图标内联 SVG（插件要离线可用），文字**复用已有的 `ui.status.refresh` 键** ——
不新增 i18n 条目，否则 `test_qq_ui_i18n_coverage` 会要求补全所有语言包。

`.wrap` 补了 `padding-bottom:76px`，否则浮动按钮会压住最后一张卡片的操作区。

**验证分两层**（因为静态检查管不了"按钮能不能点"）：

* **结构层**（进单测，不依赖浏览器）：新增 `tests/test_qq_status_refresh_reachable.py`
  （7 条），钉住"必须 fixed、默认隐藏、有 .show、有 scroll 监听、click 接的是同一个
  `load`、`.wrap` 留了底部空间、图标是内联的、文字复用已有 i18n 键"。
  并按仓库既有约定补了
  `tests/verify_status_refresh_fail_to_pass.py`：**逐个拆掉这四个必要条件都会红**
  （4/4 + 对照组绿），证明看门狗不是空过的。
* **行为层**（`.dsh-artifacts/verify-fab.py`，要 Playwright）：确认页面真的在
  **window(html)** 上滚动（否则 scroll 监听根本不触发）、顶部隐藏 / 滚到底出现且完整
  落在视口内、**点击真的执行了 `load()`**、不遮挡、无报错。

  这里踩了个坑值得记：一开始我用 `page.on('request')` 抓 `/runs` 来判断"按钮有没有接线"，
  结果**抓不到** —— `file://` 页面里的 `fetch` 会被浏览器在发出网络请求之前就拒掉。
  改成看 `load()` 的失败副作用（它 catch 后调 `showErr`，错误框会被填上）才是有效信号；
  为了排除"初始加载本来就失败"的干扰，先手动把错误框清空再点。
  另外 `position:fixed` 会把 `inline-flex` **块级化**，`getComputedStyle().display`
  返回的是 `flex` —— 这不是缺陷，是我的断言写错了。
